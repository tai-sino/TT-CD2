...
export default function CaiDat() {
  ...
  const dateFields = [
    ...
    { key: 'ngay_nhan_de_tai', label: 'Ngay nhan de tai' },
    { key: 'ngay_cham_50', label: 'Ngay cham 50%' },
    { key: 'ngay_phan_bien', label: 'Ngay phan bien' },
    { key: 'ngay_bao_ve', label: 'Ngay bao ve' },
    { key: 'ngay_ket_thuc', label: 'Ngay ket thuc' },
  ];

  return (
    <div>
      <h1 className="text-xl font-semibold text-slate-900 mb-6">Cai dat ky luan van</h1>

      <div className="bg-white rounded-lg border border-slate-200 p-6 mb-6">
        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            <label className="block text-sm font-semibold text-slate-700 mb-1">Ten ky</label>
            <input
              type="text"
              value={form.ten}
          ...
          <div className="flex gap-3">
            ...
            >
              {loading ? 'Dang luu...' : editId ? 'Luu thay doi' : 'Tao ky moi'}
            </button>
            {editId && (
              <button
                type="button"
      ...
      {kyData?.data?.length > 0 && (
        <div className="bg-white rounded-lg border border-slate-200 p-6">
          <h2 className="text-lg font-semibold text-slate-900 mb-4">Danh sach ky</h2>
          <table className="w-full">
            <thead>
              <tr className="bg-slate-50">
                <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Ten ky</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Trang thai</th>
              ...
              {kyData.data.map(ky => (
                <tr key={ky.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 border-t border-slate-100">
                    {ky.is_active ? (
                      <span className="bg-green-50 text-green-600 text-xs font-semibold px-2 py-0.5 rounded-full">Dang hoat dong</span>
                    ) : (
                      <span className="bg-slate-100 text-slate-500 text-xs font-semibold px-2 py-0.5 rounded-full">Da ket thuc</span>
                    )}
                  </td>
                  <td className="px-4 py-3 border-t border-slate-100">
                    <button
      ...
      {!isLoading && (!kyData?.data || kyData.data.length === 0) && !editId && (
        <div className="text-center py-12">
          <p className="text-slate-500 font-semibold">Chua co ky luan van</p>
          <p className="text-sm text-slate-400 mt-1">Tao ky luan van moi de bat dau quan ly.</p>
        </div>
      )}
    </div>
  );
